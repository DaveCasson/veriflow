"""Test the threshold datasource."""

import pytest

from veriflow.datasources.csv import Csv
from veriflow.datasources.inputschemas import validate_input_data


def test_validate_thresholds(xarray_thresholds: Csv) -> None:
    """Test we can validate thresholds."""
    validate_input_data(xarray_thresholds.dataset)


def test_validate_thresholds_fails_on_missing_data_type_attr(
    xarray_thresholds: Csv,
) -> None:
    """Test we can validate thresholds."""
    xarray_thresholds.dataset.attrs.pop("data_type", None)  # type: ignore[misc]
    expected_error_msg = "Input dataset is missing required 'data_type' attribute."
    with pytest.raises(
        ValueError,
        match=expected_error_msg,
    ):
        validate_input_data(xarray_thresholds.dataset)
