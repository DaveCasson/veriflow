...
def create_app(output_dataset: OutputDataset) -> Dash:
    ...
    scatter_left_stations, scatter_left_forecast = get_scatter_controls(
        default_left_pair_id,
    )
    scatter_right_stations, scatter_right_forecast = get_scatter_controls(
        default_right_pair_id,
    )
    ...
    def make_scatter_panel(panel_key, default_pair_id, stations, forecast_labels):
        return html.Div(
            [
                html.Div(
                    [
    ...
    def make_crps_panel(panel_key, default_pair_id, stations):
        return html.Div(
            [
                html.Div(
                    [
    ...
    def make_crps_figure(selected_pair_id, selected_stations):
        selected_pair = pair_lookup[selected_pair_id]
        ds = get_pair_dataset(selected_pair_id)
    ...
    def register_scatter_panel_callbacks(panel_key):
        ...
        def update_scatter_controls(selected_pair_id):
            stations, forecast_labels = get_scatter_controls(selected_pair_id)
            return (
    ...
    def register_crps_panel_callbacks(panel_key):
        ...
        def update_crps_controls(selected_pair_id):
            stations = get_crps_controls(selected_pair_id)
            return (
                [{"label": station, "value": station} for station in stations],
                [stations[0]],
            )
